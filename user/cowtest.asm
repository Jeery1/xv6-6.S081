...
malloc(uint nbytes)
{
 ...
 b86:	b7e1                	j	b4e <malloc+0x36>
      if(p->s.size == nunits)
 b88:	02e48b63          	beq	s1,a4,bbe <malloc+0xa6>
        p->s.size -= nunits;
 b8c:	4137073b          	subw	a4,a4,s3
 b90:	c798                	sw	a4,8(a5)
        p += p->s.size;
 b92:	1702                	slli	a4,a4,0x20
 ...
 ba2:	1aa73d23          	sd	a0,442(a4) # d58 <freep>
      return (void*)(p + 1);
 ...
 bbe:	6398                	ld	a4,0(a5)
 bc0:	e118                	sd	a4,0(a0)
 bc2:	bff1                	j	b9e <malloc+0x86>
  hp->s.size = nu;
 bc4:	01652423          	sw	s6,8(a0)
  free((void*)(hp + 1));
 bc8:	0541                	addi	a0,a0,16
 bca:	00000097          	auipc	ra,0x0
 bce:	ec6080e7          	jalr	-314(ra) # a90 <free>
  return freep;
 bd2:	00093503          	ld	a0,0(s2)
      if((p = morecore(nunits)) == 0)
 bd6:	d971                	beqz	a0,baa <malloc+0x92>
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 bd8:	611c                	ld	a5,0(a0)
    if(p->s.size >= nunits){
 bda:	4798                	lw	a4,8(a5)
 bdc:	fa9776e3          	bgeu	a4,s1,b88 <malloc+0x70>
    if(p == freep)
 be0:	00093703          	ld	a4,0(s2)
 be4:	853e                	mv	a0,a5
 be6:	fef719e3          	bne	a4,a5,bd8 <malloc+0xc0>
  p = sbrk(nu * sizeof(Header));
 bea:	8552                	mv	a0,s4
 bec:	00000097          	auipc	ra,0x0
 bf0:	b5e080e7          	jalr	-1186(ra) # 74a <sbrk>
  if(p == (char*)-1)
 bf4:	fd5518e3          	bne	a0,s5,bc4 <malloc+0xac>
        return 0;
 bf8:	4501                	li	a0,0
 bfa:	bf45                	j	baa <malloc+0x92>
